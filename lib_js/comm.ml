...
let process_request session (inproles : _ inp) request =
  let queue = Hashtbl.find session.queues request.request_pathspec.path_role in
  let* var =
...
let accept : type a. Server.t -> a inp service -> a io =
 fun t { sv_service_id; sv_witness = inp } ->
  ...
  in
  let service = Hashtbl.find t.services sv_service_id in
...
let send : 'a 'b. 'a ep -> ('a -> ('v, 'b) out) -> 'v -> 'b ep io =
 fun ep call v ->
  ...
  let queue = Hashtbl.find ep.ep_raw.queues out.out_role in
...
let close (ep : unit ep) =
  ...
  Server.kill_session ep.ep_raw (msg_closing ep.ep_raw)
